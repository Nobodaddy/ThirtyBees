	...
	{else}
		...
		{else}
			<div class="delivery_options_address">
						...
						<div class="delivery_options">
							{foreach $option_list as $key => $option}
								<div class="delivery_option {if ($option@index % 2)}alternate_{/if}item">
									<div>
										<table class="resume table table-bordered resp_table {if !$option.unique_carrier} hide{/if}">
											<tr>
										...
										{if !$option.unique_carrier}
											<table class="delivery_option_carrier{if isset($delivery_option[$id_address]) && $delivery_option[$id_address] == $key} selected{/if} resume table table-bordered{if $option.unique_carrier} hide{/if}">
												<tr>
													...
													<td class="white_space_normal {if $option.unique_carrier}first_item{/if}{if $first.product_list[0].carrier_list[0] eq 0} hide{/if}">
				...
				{/if}
			{/if}
			{if $conditions && $cms_id && ( version_compare($smarty.const._PS_VERSION_,'1.6.1.0','<') || (!isset($advanced_payment_api) || !$advanced_payment_api) )}
				{if $opc}
					<hr style="" />
				{/if}
		...
		{if !$opc}
				<p class="cart_navigation clearfix">
					...
					{if isset($virtual_cart) && $virtual_cart || (isset($delivery_option_list) && !empty($delivery_option_list))}
						<button type="submit" name="processCarrier" class="btn btn-default standard-checkout pull-right">
